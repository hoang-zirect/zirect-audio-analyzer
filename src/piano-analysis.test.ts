...
import { analyzeMidiPerformance, detectBpmFromOnsets, extractTopMelody, inferChords, melodyContourNotes, parseMidi, scoreMelody, type MidiNote, type PianoReport } from "./piano-analysis";
...
describe("MIDI performance metrics", () => {
  it("calculates melody and left-hand metrics from the selected tracks", () => {
    const notes = [
      { ...note(72, 0, .4), track: 1 }, { ...note(79, 1, .4), track: 1 }, { ...note(76, 2, .4), track: 1 },
      { ...note(48, 0, 1), track: 0 }, { ...note(52, 0, 1), track: 0 }, { ...note(50, 2, 1), track: 0 },
    ];
    const report = { bpm: 60, timeSignature: "4/4", key: "C major", duration: 4, noteRange: "C3–G5", notes, chords: [{ start: 0, end: 2, name: "C", roman: "I" }, { start: 2, end: 4, name: "Dm", roman: "ii" }], progression: "I – ii", metrics: [], score: 0 } satisfies PianoReport;
    const metrics = analyzeMidiPerformance(report, 1, 0);
    expect(metrics.notesPerMinute).toBe(45); expect(metrics.leapsOver5).toBe(1); expect(metrics.leapsOver7).toBe(0);
    expect(metrics.range).toBe(7); expect(metrics.leftNotesPerChord).toBe(1.5); expect(metrics.chordChangesPerMinute).toBe(30);
  });
});
